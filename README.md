...
See `CLAUDE.md` for the full plan. Summary: a **task** is a one-time item (`date` null means it lives in the weekly sidebar, tagged with a `week` key instead); a **template** is a recurring routine (daily/weekly-on-days/monthly-on-day, optionally every N weeks, optionally with an end date) whose occurrences are materialized virtually at render time rather than stored — completing one just writes to `completions`, editing/moving one creates a real `template_exception` task for that date. Week keys are the week's start date under the current week-start setting (Sun or Mon), not ISO week numbers — simpler and stays consistent with what the week view actually displays.

**An end date on a recurring task** — "every Sunday and Wednesday until October 22" — is `recurrence.until`, an inclusive `YYYY-MM-DD` last day set under *Routines & settings → the task → Ends*. It's a ceiling over every frequency, checked once at the top of `templateOccursOn`, and only present on templates that have one, so nothing saved before it existed needed migrating. `outstandingOccurrence` respects it too, so a task set to carry forward stops carrying once it ends. Past its end date the routines list reads "ended Aug 1" rather than "until Aug 1". The two brief Edge Functions bundle their own copy of the check and need redeploying for briefs to honor it.

**Monthly-recurring tasks** are a separate concept from templates (which never carry over). A task in the "This month" bucket with `recurMonthly:true` and a stable `seriesId` (its own id at creation) recurs like this: `ensureMonthlyRecurring()` (in `afterLoad`, after `runRollover`) adds a fresh "native" copy — `rolled_from:null`, `month` = current — whenever the current month lacks one, while a forgotten copy from a past month has already been carried into the current month by the normal rollover with `rolled_from` set. So a forgotten month and this month both show, the forgotten one flagged (repeat icon + orange dot). Idempotent: it only creates when no native exists. Unchecking "repeat every month" runs `stopMonthlySeries()`, clearing the flag across every instance so nothing regenerates.
...
- **Adding a meal** is one type-to-search box: what you type filters your recipe database live (click a match to add it *linked* to that recipe), and an always-present "Add “…” as a custom dish" row means typing + Enter adds a plain meal whether or not a recipe matches. The picker surfaces load state explicitly — "Loading…", the actual error string on failure (with a retry link), or "loaded fine, 0 recipes" — so a blank recipe list is diagnosable rather than silently empty.
- **The day view says when the planner is ahead of it.** Because the calendar import is a snapshot *and* per-week, importing one week never keeps the next in sync — meals planned in the dinner planner would otherwise just never appear. When the day in view has planned meals the agenda doesn't have, a nudge above "Add a meal" names them; **Import** takes that day alone (never the whole week, which would touch other days unasked), and **×** records the date in `settings.calNudgeDismissed` so a day deliberately kept out stops asking — importing that day clears the dismissal. Detection compares **titles**, so a side/dessert added to an already-imported day isn't flagged: those flatten into the dinner meal's `extras`, and signature-matching would re-flag a meal that is visibly already there. `plannedMealsForDate()` is the single source for what a planned day contains — the week import, the day import, and this check all read through it so they cannot drift. `afterLoad()` warms the (cached) planner read, which until now only happened when the picker first opened.
...
The header's grid button lists the sibling apps (order set by Bri: dinner, law, restock, perfume, sewing, time — no hub) plus her own saved **quick links**. The suite list comes from the hub's `apps.json` — one source of truth for every app that lists the others, fetched same-origin so there's no CORS or auth and it works in guest mode. `SUITE_APPS_FALLBACK` is the hand-synced copy used only when that fetch fails: this is an installed PWA, so offline should leave a stale menu rather than an empty one. The agenda filters itself out by id, and re-fetches when the menu opens so a newly registered app appears without a reload. Everything in the menu is a real `<a target="_blank">`, so clicking is plain user-initiated navigation.
...
Term matching tolerates spacing drift (`termToRegex` joins a term's words with `\s*`), because a keyword saved as "hair cut" is typed as "haircut". Word boundaries stay mandatory, so "therapy" still refuses "therapeutic" — the looseness is only ever about gaps between words.
...
**Asking for a link is answered without a round trip.** `matchQuickLink()` runs in `quickParseAdd`/`sendChatMessage` *before* any await, still inside the Enter or click, because a `window.open()` issued after an await is exactly what popup blockers exist to stop. It needs a single unambiguous hit — two links matching means falling through to the assistant, which can ask. It also refuses anything carrying a day or a time (`looksScheduled()`): "therapy Thursday at 3" contains "therapy" but is an appointment, and the keyword match alone would have opened the portal instead of creating the task. Scheduling language is exactly the case where the whole sentence has to be weighed, so it goes to the model. The assistant also has an `open_link` action for phrasings the keywords miss; that path is necessarily post-gesture, so it tries to open and, failing that, renders the link as one tap rather than silently doing nothing.
...
A header control is the whole footprint: an ordinary icon button when idle, a live chip (`1:04 · Footnote pass`) when running, ticking every 30s. Tapping it opens a description field plus a category list (unarchived only, category-only — sub-types get refined in the tracker) or, while running, "stop and log it". The description is captured *before* the category because picking a category is what starts the timer; it flows onto the entry and shows in the chip. While running it stays editable in the same menu — the save re-reads first so `started_at` and category survive, and `stopTracking` awaits any in-flight save, since blurring the field to click stop fires both at once and the entry must log what was actually typed. The menu skips its lazy re-render while that field has focus rather than replacing the node mid-edit. The clock on a task row is the same action with the title pre-filled and the tracker's own `settings.lastTimerCat` as the category.
...
**Purchases.** `log_restock_purchase` mirrors restock's own `confirmBuy()`: append `{date, price, store, size, on_sale}` to the product's `purchases`, add `qty` to `on_hand`, mark the staple stocked and clear its snooze. Two deliberate differences from the modal over there. It prefills price/store from the last purchase because a human then confirms the form; nothing confirms an assistant action, so price and store are recorded **only when actually spoken** — an invented price would quietly pollute the price history the sale-pattern tracking reads. And the prompt has to hold `qty` (a count of packages) apart from `size` (the package size), so "1lb of Good Folks coffee" is qty 1 / size "1lb", never qty 1. A purchase also closes any open "Buy X" task for that staple — which is *not* the mirror of ticking that task off, which deliberately leaves restock alone: buying something and putting it away are different moments, but saying "I bought it" is not.

**Out → a task to buy it.** `syncRestockOutTasks()` mirrors every staple sitting at `out` into the weekly sidebar as "Buy X" (`source: 'restock'`, `restock_staple_id`), regardless of where it was marked — the AI bar here or a tap in the restock app. It runs on load, after an assistant restock write, and on tab refocus (which re-reads restock, so an Out tapped on the phone surfaces on return rather than only after a full reload). `settings.restockTasked` records which staples already produced a task, so checking that task off — or deleting it — doesn't make it reappear next load; the record clears once the staple isn't out, so the *next* time it runs out earns a fresh task. Unfinished sidebar tasks already roll to the following week, so a staple that stays out keeps nagging without extra code.
